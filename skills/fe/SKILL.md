...
### 4대 원칙
1. **가독성** — 코드를 읽는 사람의 맥락을 줄여라. 구현 상세를 추상화하고, 위에서 아래로 자연스럽게 읽히게 작성한다.
2. **예측 가능성** — 함수/컴포넌트 이름만 보고 동작을 예측할 수 있어야 한다. 숨은 사이드 이펙트를 제거하고 일관된 패턴을 유지한다.
3. **응집도** — 함께 수정되는 코드는 함께 둔다. 변경 범위를 찾기 쉽고 사이드 이펙트를 예측할 수 있게 한다.
4. **결합도** — 모듈 간 의존성을 최소화한다. 한 모듈의 변경이 다른 모듈에 미치는 영향을 줄인다.
